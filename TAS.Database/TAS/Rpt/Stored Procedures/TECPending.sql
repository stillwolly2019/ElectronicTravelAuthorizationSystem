...
-- =============================================
CREATE PROCEDURE [Rpt].[TECPending]
...
FROM [AttendanceMonitoringSystem-Branch].Lkp.ReportSubscription s 
),
SupervisorList
AS (
SELECT DISTINCT  a.EmployeeID,a.PersonnelNumber,a.[Name],a.EndDate,
REPLACE( REPLACE (ManagerEmail,'<xx>',''),'</xx>','') as ManagerEmail
    FROM  [AttendanceMonitoringSystem-Branch].Att.EmployeesInfo a
...
ta.[TravelersName],
LEFT(s.ManagerEmail, LEN(s.ManagerEmail) - 1) [Supervisor Email],
